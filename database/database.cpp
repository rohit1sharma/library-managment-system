...
status database::getUserDetails(const shared_ptr<user> &user)
{
    // Code for fetching user details from the database can be added here
    try
    {
        if (user)
        {
            string email = user->get_email().value_or("");
            const string qer = "SELECT name, email FROM users WHERE email = '" + email + "'";
            vector<vector<string>> result = query(qer);
            if (!result.empty())
            {
                user->set_name(result[0][0]);
                user->set_email(result[0][1]);
            }
        }
        else
        {
